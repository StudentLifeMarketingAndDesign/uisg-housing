...
//			echo "id=$id\n";
	...
	**/
	...
	static function make_link($action, $id){
		return BASE_URL . "/admin/" . self::$url_segment . "/{$action}/$id";
	}
	
	
	/**
	 * returns true of blogentries are set up with has_one(Member) authors
	 * @todo could we make this test not dependent on blogplus?
	 */
	static function use_member_authors(){
		return class_exists('BlogExtensions') && BlogExtensions::$enable_members_as_authors;
	}
	...
	static function use_categories(){
		return class_exists('BlogExtensions') && BlogExtensions::$enable_blog_categories;
	}
